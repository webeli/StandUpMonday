...
class LoggedIn extends Component {
  ...
  constructor(props) {
    ...
    props.roomStore.onLogs(this.props.routing.location.pathname);
  }
  ...
  render() {
    ...
    const mappedLogs = this.props.roomStore.logs.map(log => {
      return (
        <Col key={log.key}>
          <b>{log.value.displayName}</b> {log.value.body} <b>{log.value.timestamp}</b>
        </Col>
      )
    })
    ...
    return this.props.roomStore.roomFound && (
      <Row type="flex" justify="space-between" align="middle" style={{ textAlign: 'center', marginTop: '20px' }}>
        <Col xs={{ span: 14, offset: 8 }} lg={{ span: 8, offset: 0 }}>
          {mappedAttendees}
        </Col>
        ...
        <Col xs={{ span: 24, offset: 0 }} lg={{ span: 8, offset: 8 }} style={{ marginTop: '20px' }}>
          {mappedLogs}
        </Col>
